...
export const CopyButton = ({ text }: { text: string; }) => {
    ...
    return (
        <button
            onClick={handleCopy}
            className={`absolute top-2 right-2 p-2 rounded-lg transition-all duration-200 z-50 bg-card/80 text-muted-foreground border border-border shadow-sm backdrop-blur-md opacity-0 group-hover:opacity-100 focus:opacity-100 hover:text-foreground hover:scale-110 active:scale-100 cursor-pointer tooltip tooltip-left`}
