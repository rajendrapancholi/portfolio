...
const Thumbnail = ({
  ...
  const onFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    let uploadedUrl;
    if (type === "project") {
      uploadedUrl = await handleCloudinaryAdminUpload(file, currentUrl);
    } else if (type === "blog") {
      uploadedUrl = await handleCloudinaryBlogUpload(file, currentUrl);
    }
  ...
  const onDelete = async () => {
    ...
    let success = false;
    if (type === "project") {
      success = await handleCloudinaryAdminDelete(currentUrl);
    } else if (type === "blog") {
      success = await handleCloudinaryBlogDelete(currentUrl);
    }
      ...
      <div className="relative w-full max-w-4xl bg-[#0f172a] border border-white/10 rounded-[2.5rem] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300">
        ...
        <div className="grid md:grid-cols-2 gap-0">
            ...
            <label className="group relative flex flex-col items-center justify-center w-full h-64 border-2 border-dashed border-white/10 hover:border-cyan-500/50 bg-white/2 hover:bg-cyan-500/2 rounded-3xl transition-all cursor-pointer overflow-hidden">
              {isUploading ? (
                ...
                <div className="text-center p-6">
                  <CloudUpload
                    className="mx-auto text-gray-500 group-hover:text-cyan-400 transition-colors mb-4"
                    size={48}
                  />
          ...
          <div className="p-8 md:p-12 bg-black/20 flex flex-col justify-center">
            <span className="text-[10px] font-black text-gray-500 uppercase tracking-[0.2em] mb-6">
              Live Card Preview
            </span>
            ...
            <div className="mt-8 flex items-start gap-3 p-4 bg-cyan-500/5 border border-cyan-500/10 rounded-2xl">
              ...
              <p className="text-xs text-gray-400 leading-relaxed">
                Content curated and optimized for the <b>Rajendra Pancholi</b>{" "}
                ecosystem for premium web delivery.
              </p>
            </div>
          </div>
        </div>
