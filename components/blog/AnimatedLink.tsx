...
export default function AnimatedLink({
  ...
  return (
    ...
    >
      ...
      >
        <div
          className={`absolute inset-0 bg-linear-to-r from-cyan-500/30 dark:from-cyan-500/10  to-transparent  group-hover:opacity-100 transition-opacity ${pathname ? 'opacity-100' : 'opacity-0'}`}
        />
        ...
        <div
          className={`absolute right-3 ml-auto w-1.5 h-1.5 rounded-full bg-cyan-500 transition-opacity shadow-[0_0_8px_#06b6d4] ${pathname ? 'opacity-100' : 'opacity-0'}`}
        />
      </Link>
    </MotionDiv>
  );
